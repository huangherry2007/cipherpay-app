...
async function airdropSol(connection: Connection, pubkey: PublicKey, sol = 2) {
...
(async function main() {
  ...
  const amountAtoms = 1_000_000n; // 0.001 SOL
  ...
  await wrapWSOLIfNeeded(
    connection,
    user.publicKey,
    Number(publicSignals.amount as any), // lamports needed
    user.publicKey,
    // adapt our send(tx) helper to the expected signature
    async (tx: Transaction) => send(tx)
  );
  ...
  const vaultWsolAta = getAssociatedTokenAddressSync(
    WSOL_MINT, vaultAuthorityPda, true, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID
  );
